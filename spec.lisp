...
(defmethod get-field-dim ((operand string) spec)
  (declare (ignore spec))
  (make-instance 'field-dim :offset 0
                            :size (length operand)
                            :filter operand))

(defmethod get-field-dim ((operand symbol) spec)
  (let ((field (gethash operand (spec-fields spec))))
    (make-instance 'field-dim :offset (db-field-offset field)
